...
class phaseDraft1846NoGame {
  ...
  function init() {
    ...
    else $this->r->mChan("A new draft can now begin.");
  }
  function cmdjoin($from, $args) {
    if(isset($this->r->players[$from])) {
      $this->r->mChan("$from: You have already joined the current draft.");
      return;
    }
    ...
    $this->r->players[$from] = new Draft1846Player($this->r, $from);
  ...
  function cmdpart($from, $args) {
    if(!(isset($this->r->players[$from]))) {
      $this->r->mChan("$from: You are not in the current draft.");
      return;
    }
  ...
  function cmdstart($from, $args) {
    if(!(isset($this->r->players[$from]))) {
      $this->r->mChan("$from: You must be in the current draft to start it.");
      return;
    }
    $playerCount = count($this->r->players);
    if($playerCount < $this->minPlayers) {
      $this->r->mChan("$from: Sorry, only $playerCount player(s) have joined. {$this->r->game} requires {$this->minPlayers}-{$this->maxPlayers} players to start. Current players are: ".$this->r->playerList().".");
      return;
    }
